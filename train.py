...
def categoryFromOutput(pre, lab):
    ...
    pre_seq = pre.argmax(dim=1)
    lab_seq = lab.squeeze(dim =0)
    equallist = [int(pre_seq[i]==lab_seq[i]) for i in range(pre_seq.size()[0]) ]

    predict = [id2tag_dict[c.item()] for c in pre_seq]
    label = [id2tag_dict[c.item()] for c in lab_seq]
